...
- SD card: https://www.amazon.com/dp/B07FCMBLV6
    * A high quality A2/U3 rated SD card is highly recommended for trouble free operation. Sandisk Extreme or Samsung Pro Plus have been tested and work well. These SD cards are generally shipped with exFAT formatting, which will not work with this project. The SD card must be formatted to fat32.
...
### Recording Format:
Similar to how a dashcam/bodycam works, this code records audio to 24bit 44khz raw PCM files which are not prone to corruption on sudden power loss. When power is cutoff, typically only the last 0-200msec of audio are lost; in a rare worst case event the last ~1.7sec may be lost. The files are named "rec_X-Y.24bit.raw", where X is the recording session, and Y is the audio segment. Both X and Y increment to allow sequential reconstruction recordings. A new segment file is created within a session after every ~2min of recording, or after the new segment file button is pressed. Code design ensures no gaps in audio between segment files. New sessions are created after power cycles.

The code makes use of a DSP filter with HPF, RMS audio compressor, and clipping limiter, which are tuned for recording "ambient voice and noise". Users can modify DSP parameters to suit their specific needs by editing values of the 7 `#defines` located at the top of the code. These parameters have comments on usage for those not familiar with DSP technical details. In general the tuning the DSP will involve balancing distant noises with background hiss to suit personal tastes.
...
Note that raw2wav.exe does not actually modify the audio content; it simply adds wav headers to the raw data and saves the files with a `*.wav` extension. Alternatively, you can do your conversions using a program like Audacity to "import raw data", "24bit unsigned", "little endian", "mono", with "44100hz" bit rate, and export to a wav file. If you do not want to use the included executable, you can compile the source using python. The compile command I used is written at the top of `raw2wav.py`.

### Logging:
A `manifest.log` file is saved to the sd card for troubleshooting purposes. Below is an example of lines that may be appended to the log (note that CLOSE and METADATA lines are written only when either a segment last longer than ~2min, or when the new segment button is used):
...
- PEAK: Tells you the highest 24bit dynamic audio level recorded in that file segment ($0$ to $8,388,607$). During typical use this should be roughly `2000000` to `6000000`, indicating proper AGC tuning for the application. If this value is very low (`~300,000`) or high (`~8000000`), it indicates poor utilization of the available dynamic range from the audio engine. Your DSP parameters (or choice of mic) may not be well suited to your application, and tweaking these items may help.
- OVERRUNS: The number of dropped audio blocks. This value should remain 0. Consistent non-zero values indicate an out of spec (not A2/U3) SD card that is struggling to keep up with sustained writes.
- AVG_QUEUE: Represents the running average depth of the queue over time. A healthy value sits between 0.5 and 2.0. If this numbers climbs up to 15.0 or 30.0 on a file, it indicates high SD card write latency, which may affect the reliability of recordings.
...
Files for a printable enclosure are provided in this repo. One of each part should be printed. Print the "agcRecorderTopWindscreen.stl" if you want to use a 10mm foam or furry wind screen on your mic. ABS or PETG is recommended for the top, bottom, and shim. Clear PETG filament or clear SLA resin should be used for the lens. I recommend using T-7000 adhesive on all of the glue joints described below.

Test fit all of the parts before wiring and assembling with glue. Make sure you can fit a toothpick or similar insulated rod through the 2 button access holes on the top of the case; cut them larger if needed. Break off the unused pins on one side of your button to prevent shorting. Insert the button into the enclosure top with pins facing the mic hole. Insert the lens into the square hole next to the button. Insert the mic board into the enclosure top, pin holes first, with the mic hole facing outward. Insert the ESP32 board into the enclosure top. Insert the SD card into it's spot on the bottom of the case, with the bare side of the board pointing downward. Snap the top and bottom halves together, making sure they fully engage. There should be a ~0.5mm gap bewteen them when fully seated. If this gap is crooked or too large, adjust fit/reprint as needed. If everything fits well, disassemble and move on to wiring.

<img src="https://github.com/truglodite/fpvGoggleAudioRecorderESP32/blob/main/images/wiring.jpg" width="600">

Tidy wire routing is important as the enclosure is compact. Silicone stranded 30awg wire or smaller is recommended. Cut the wires long enough to permit assembly without tension, but not so long that assembly may pinch a wire. Start with soldering wires to the back side of the ESP32 board (opposite of boot/reset buttons). The 3 GND wires and 2 3v3 wires can be solder on the larger pads of the ESP32; it's easiest to strip and twist these wires together before tinning and soldering the bundles on the pads. Cut/solder wires to the button pins and coat the joints with glue. Cut/solder wires to the chip side of the microphone, adding a short jumper wire between the SEL and GND pins. Cut/solder wires to the top (card side) of the SD card board. Power up the circuit and verify proper operation after soldering is complete. If it records audio properly, proceed with the rest of the assembly.
